    ...
    <DocsLayout>
      <div className="space-y-12">
        ...
        <section className="space-y-4">
          ...
          >
            Engineered for precision and scale. The platform utilizes a layered architecture that strictly separates 
            source data, business logic, and transport mechanisms.
          </motion.p>
        </section>
          ...
          <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-3">
               <div className="p-4 rounded-2xl bg-zinc-900/50 border border-zinc-800/50">
                  <span className="text-[10px] font-black uppercase text-blue-500 tracking-widest mb-1 block">Level 1</span>
                  <h3 className="font-bold text-zinc-200 mb-1">In-Memory</h3>
                  <p className="text-xs text-zinc-500">Ultra-low latency cache local to each server instance.</p>
               </div>
               <div className="p-4 rounded-2xl bg-zinc-900/50 border border-zinc-800/50">
                  <span className="text-[10px] font-black uppercase text-indigo-500 tracking-widest mb-1 block">Level 2</span>
                  <h3 className="font-bold text-zinc-200 mb-1">Redis</h3>
                  <p className="text-xs text-zinc-500">Distributed cache for shared state across horizontally scaled pods.</p>
               </div>
               <div className="p-4 rounded-2xl bg-zinc-900/50 border border-zinc-800/50">
                  <span className="text-[10px] font-black uppercase text-emerald-500 tracking-widest mb-1 block">Monitoring</span>
                  <h3 className="font-bold text-zinc-200 mb-1">X-Cache</h3>
                  <p className="text-xs text-zinc-500">Full observability via custom headers (hit-memory, hit-redis, miss).</p>
               </div>
            </div>
        ...
        <section className="space-y-4">
           ...
           <div className="grid gap-3 text-sm text-zinc-400">
             <div className="flex items-center gap-3 p-4 rounded-2xl border border-zinc-800 bg-zinc-900/10">
                <span className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-500/10 text-[10px] font-bold text-emerald-500">01</span>
                <span>Deterministic hashing of all generated JSON and SQL artifacts.</span>
             </div>
             <div className="flex items-center gap-3 p-4 rounded-2xl border border-zinc-800 bg-zinc-900/10">
                <span className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-500/10 text-[10px] font-bold text-emerald-500">02</span>
                <span>Strict TypeScript compilation and Vitest coverage for core domain logic.</span>
             </div>
             <div className="flex items-center gap-3 p-4 rounded-2xl border border-zinc-800 bg-zinc-900/10">
                <span className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-500/10 text-[10px] font-bold text-emerald-500">03</span>
